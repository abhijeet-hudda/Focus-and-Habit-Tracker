...
export default function Dashboard() {
  ...
  const createMutation = useMutation({
    mutationFn: (newActivity) =>
      api.post("/activities/create-Activity", newActivity),
    onSuccess: () => {
      queryClient.invalidateQueries(["activities"]);
      setForm({ activityName: "", duration: "", category: "Work" });
      toast.success("Activity added successfully!", { theme: "dark" });
    },
    onError: (error) => {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(["activities"]);
      toast.success("Activity deleted!", { theme: "dark" });
    },
    onError: (error) => {
      toast.error(
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!form.activityName || !form.duration) {
      toast.info("Please fill all fields", { theme: "dark" });
      return;
    }
          ...
          <div className="bg-card border border-border rounded-2xl p-6 shadow-sm overflow-y-auto custom-scrollbar h-fit max-h-full">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
                  {CATEGORIES.map((cat) => (
                    <button
                      ...
                      className={`
                      flex flex-col items-center justify-center gap-1 py-3 rounded-xl border transition-all disabled:opacity-50 cursor-pointer disabled:cursor-not-allowed
                      ${
              ...
              <motion.button
                type="submit"
                disabled={createMutation.isPending}
                className="w-full bg-primary cursor-pointer text-primary-foreground font-bold py-3.5 rounded-xl hover:opacity-90 transition-all active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed mt-4 flex items-center justify-center gap-2"
              >
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          <div
                            className={`w-10 h-10 rounded-lg flex items-center justify-center ${style.bg} ${style.color} border ${style.border}`}
                          >
                        ...
                        <motion.button
                          ...
                          className="p-2 cursor-pointer text-muted-foreground hover:text-destructive hover:bg-destructive/30 rounded-lg opacity-0 group-hover:opacity-100 transition-all disabled:opacity-70 disabled:cursor-not-allowed"
                        >
